 ...
 * the bot at once, each with its own configuration. Every command is
 * open to everyone by default — there is no plan/payment gate.
 */
...
const { synchronizeData } = require("./database");
    ...
    try {
        ...
        if (m.key && !fromMe && (body || isMediaMsg)) {
            ...
            if (config.antidelete && isMediaMsg) {
                (async () => {
                    try {
                        const { getMediaFromMessage } = require("./media");
                        ...
                        if (media) {
                            ...
                            if (config.mongoUrl) {
                                const { saveMedia } = require("./mediaVault");
        ...
        // --- Plugin engine (commands live in /command — every command is open to everyone) ---
        if (isCmd && command) {
            const pluginFolder = path.join(__dirname, "command");
